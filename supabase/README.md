...
| 15 | `billing.sql` | `subscriptions` and `stripe_events` — Phase 11. Read-only to clients by design; see `docs/BILLING.md`. | Only if you take payment. |
| 16 | `community.sql` | `community_profiles`, `shared_setups`, `content_reports` and the `leaderboard()` / `browse_setups()` functions — Phase 10. Opt-in; nothing is visible until a user switches it on. | Optional; the Community page says so if it's absent. |
